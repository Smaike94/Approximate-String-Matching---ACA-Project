...
void sequences_reader(char *pattern, kseq_t *seq, double k_edit_dist_perc);
...
void string_compare(char *p, char *t, int nr, int nc, int ncol_prev, cell *mat, cell *mat_p ,bool show_mat, double *time_matrix);
...
void reconstruct_path(char *p, char *t, char *t_prev,int i, int j, char *edit_path, int nr, int nc, cell *matp, int ncol_prev, cell *mat);
...
int main(int argc, char *argv[])
{
    ...
    while((l_pattern = kseq_read(seq_pattern)) >= 0){
    	...
    	if ((l = kseq_read(seq)) >= 0){
    		sequences_reader(pattern, seq, k_edit_dist_perc);
    	}
...
void sequences_reader(char *pattern, kseq_t *seq, double k_edit_dist_perc){
    ...
    bool show = false;
    double totalTime_part_to_parallelize = 0.0;
    ...
    do{
        ...
        for(int index_char=0; index_char < length_sequence; index_char += num_char_subsequence){
			...
			string_compare(pattern, text_curr, nrows, ncol_curr, ncol_prev, Mcurr, Mprev, show, &totalTime_part_to_parallelize);
			...
			int max_lenght_substring_matched = strlen(pattern) + k_edit_dist + 1;
			int max_rows = 2 * num_submatch; // Because  each pair of edit path and substring will be saved

			char *substring_matched = calloc(max_rows * max_lenght_substring_matched, sizeof(char));
			susbtrings_matched(pattern, text_curr, text_prev, submatch_indexes,
					num_submatch, nrows, ncol_curr, Mcurr, max_rows,
					max_lenght_substring_matched, Mprev, ncol_prev,
					substring_matched);

			save_subtrings_matched(pattern, k_edit_dist, length_sequence, max_rows, max_lenght_substring_matched, substring_matched,Mprev);
			free(substring_matched);
	    	...
	    	totalTime_part_to_parallelize += (finalTime_substrings_searching - initialTime_substrings_searching);
	...
	printf("\n\nELAPSED TOTAL TIME SEQUENCE: %f [s]\nPATTERN LENGTH [%li]\nSEQ LENGTH[%li]\nK_EDIT_DIST[%i]\n\n",
			...
			strlen(pattern), length_sequence, k_edit_dist);

	FILE *fptr_results;
	fptr_results = fopen("./results_time.txt", "a");
	fprintf(fptr_results,"\n\nELAPSED TOTAL TIME SEQUENCE: %f [s]\nELAPSE TIME PARALLELIZE: %f [s]\nPATTERN LENGTH [%li]\nSEQ LENGTH[%li]\nK_EDIT_DIST[%i]\n\n",
			finalTime_searching_sequence - initialTime_searching_sequence,
			totalTime_part_to_parallelize, strlen(pattern), length_sequence,
			k_edit_dist);
	fclose(fptr_results);
...
void string_compare(char *p, char *t, int nr, int nc, int ncol_prev, cell *mat, cell *mat_p ,bool show_mat, double *time_matrix)
{
    ...
    for (i = 1; i < nr; i++)
    {
        for (j = 1; j < nc; j++)
        {
            ...
            int match_cost = (p[i - 1] == t[j + offset_text_index]) ? 0 : 1;
    ...
    *time_matrix += (finalTime_edit_distance_matrix-initialTime_edit_distance_matrix);
...
void reconstruct_path(char *p, char *t, char *t_prev,int i, int j, char *edit_path, int nr, int nc, cell *matp, int ncol_prev, cell *mat)
{
    ...
    {
        reconstruct_path(p, t, t_prev, i - 1, j - 1, edit_path, nr, nc, matp, ncol_prev, mat);
        match_out(p, t, t_prev, i, j, matp, edit_path);
        return;
    }
    if (mat[mat_index].parent == INSERT)
    {
        reconstruct_path(p, t, t_prev, i, j - 1, edit_path, nr, nc, matp, ncol_prev, mat);
        insert_out(t, j, edit_path);
        return;
    }
    if (mat[mat_index].parent == DELETE)
    {
        reconstruct_path(p, t, t_prev, i - 1, j, edit_path, nr, nc, matp, ncol_prev, mat);
        delete_out(p, i, edit_path);
        return;
    }
}
